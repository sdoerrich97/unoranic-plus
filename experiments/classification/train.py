...
@description:
Training of unORANIC+ for a classification objective.
...
class Trainer:
    def __init__(self, configuration):
        ...
        self.data_loader = DataLoaderCustom(self.dataset, self.data_path, self.img_size, self.batch_size, train=True)
        ...
        if self.architecture_encoder == 'unoranic':
            ...
            self.model.encoder.load_state_dict(checkpoint['encoder_anatomy'])
    ...
    def run_iteration(self, mode, data_loader, epoch):
        ...
        for i, (_, Y, X, _) in enumerate(data_loader):
    ...
    def save_model(self, epoch: int, save_idx=50):
        ...
        :param epoch: Current epoch.
        :param save_best: Save the current best model.
        :param save_idx: Which epochs to save.
        """

        # Create the checkpoint
        if self.architecture_encoder == 'resnet18' or self.architecture_encoder == 'resnet50' or self.architecture_encoder == 'unoranic':
            checkpoint = {
...
def main(training_configuration=None):
    ...
    if training_configuration is not None:
        if training_configuration["resume_training"]["resume"]:
            ...
            wandb.init(project="unoranic+_classification_training", id=training_configuration["resume_training"]["wandb_id"], resume="allow", config=training_configuration)

        else:
            # Initialize a weights & biases project for a training run with the given training configuration
            wandb.init(project="unoranic+_classification_training", name=f"{training_configuration['dataset']}-{training_configuration['architecture_encoder']}", config=training_configuration)

    # Run the hyperparameter optimization run
    else:
        # Initialize a weights & biases project for a hyperparameter optimization run
        wandb.init(project="unoranic+_classification_training_sweep")
...
if __name__ == '__main__':
    ...
    with open(config_file, 'r') as f:
        ...
        if sweep:
            # Configure the sweep
            sweep_id = wandb.sweep(sweep=config['hyperparameter_tuning'], project="unoranic+_classification_training_sweep")
